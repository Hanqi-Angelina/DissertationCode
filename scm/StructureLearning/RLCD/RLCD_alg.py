...
from StructureLearning.RLCD.PC_CovRank import pc_true_cov_rank
...
def RLCD(
    ...
    if parameters['stages']>=1:
        if not parameters['sample']:
            ...
            # elif parameters['stage1_method'] == 'pc':
            #     # added the PC version here (as this is what is said in their paper)
            #     G, edges = pc_true_cov_rank(df_x.to_numpy(), parameters['ranktest_method'])
            #     Adj_stage1 = process_pc_result(G.graph)  # possible implementation of PC
            #     partition = getPartition(xvars, abs(Adj_stage1), parameters['stage1_partition_thres'])
        else:
            if parameters['stage1_method']=='all':
                Adj_stage1 = np.ones((len(xvars),len(xvars)))
                ...
                try:
                    # will start the JVM at the beginning, shold not be started here, just include in case needed
                    if not jpype.isJVMStarted():
                        # Get the path to the current script (RLCD_alg.py)
                        script_dir = os.path.dirname(os.path.abspath(__file__))

                        # Navigate up to the project root directory
                        project_root = os.path.abspath(os.path.join(script_dir, '..', '..'))
                        
                        # Construct the full path to the tetrad-current.jar file
                        tetrad_jar_path = os.path.join(project_root, 'pytetrad', 'tetrad-current.jar')
                        
                        if not os.path.exists(tetrad_jar_path):
                            print("Error: tetrad-current.jar not found at the expected path.")
                            raise FileNotFoundError("Could not find tetrad-current.jar")
                        
                        jpype.startJVM(jpype.getDefaultJVMPath(), classpath=[tetrad_jar_path])
                ...
                # from pytetrad.TetradSearch import TetradSearch
...
def AdjToGraph(Adj, varnames_for_Adj):
    ...
    for i in range(len(Adj)):
        for j in range(i+1, len(Adj)):
            # seems that first case -> undirected edges and the other case -> directed edges
