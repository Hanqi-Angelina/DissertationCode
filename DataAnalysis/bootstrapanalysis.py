...
# --- inputs & summary file ---
IN_DIR = Path(SCRIPT_DIR) / "Bootstrap"
files = sorted(IN_DIR.glob("Bootstrap_*.csv"))
curr_sum = pd.read_csv(f"{SCRIPT_DIR}/Bootstrap/sum.csv")
...
for f in files:
    ...
    m = re.search(r"Bootstrap_(\d+)\.csv$", f.name)
    if not m:
    ...
    # optional sanity check if you ever worry about order
...
# updated n_latents
curr_sum.to_csv(f"{SCRIPT_DIR}/Bootstrap/sum_new.csv")

# merge & summarize
merged_counts = merge_counts(dict_list)
df_summary = counts_to_df(merged_counts, vars)
df_summary.to_csv(f"{SCRIPT_DIR}/Bootstrap/summary_prop.csv")
