...
from AnalysisUtils import run_RLCD_on_data, get_col_dict, bootstrap_indices

# Absolute paths & node-local scratch (if available)
WORK_DIR = os.environ.get("SLURM_TMPDIR", os.getcwd())
os.makedirs(WORK_DIR, exist_ok=True)

save_folder = f'{SCRIPT_DIR}/Bootstrap'
...
for run_id in run_ids:
    ...
    new_ids = bootstrap_indices(n=n, seed=run_id)
    ...
    try:
        ...
        pd.DataFrame(best_perm_df).to_csv(f'{save_folder}/Bootstrap_{run_id}.csv', index=True)
