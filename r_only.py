...
def to_signed_cpdag(A01: pd.DataFrame) -> pd.DataFrame:
    A = A01.copy().astype(int)
    S = pd.DataFrame(0, index=A.index, columns=A.columns, dtype=int)
    for i in A.index:
        for j in A.columns:
            if i == j : 
                continue
            if A.loc[i, j] == 1 and A.loc[j, i] == 0:
                S.loc[i, j], S.loc[j, i] = -1, 1   
            elif A.loc[i, j] == 1 and A.loc[j, i] == 1:
                S.loc[i, j] = S.loc[j, i] = -1      
    return S
...
def main_orchestrator():
    ...
    final_output_path = os.path.join(os.getcwd(), f'evaluation_summary_{num_runs}_{noise_type}_r.csv')
    ...
    for run_idx in range(num_runs):
        for sample_size in sample_sizes:
            for scenario_name in scenario_classes.keys():
                print(f"  -> Generating data for {scenario_name} (N={sample_size}, Run={run_idx+1})", flush=True)

                df_x, true_adj_df, n_x = data_generator(
                    scenario_classes=scenario_classes, scenario_name=scenario_name, 
                    run_idx=run_idx+1, sample_size=sample_size, noise_type=noise_type
                )
                
                # 2. Loop through all methods
                for method_name, worker_script in methods.items():
                    key = (scenario_name, method_name, sample_size, run_idx + 1)
                    if key in done:
                    ...
                    try:
                        ...
                        temp_csv_input  = os.path.join(WORK_DIR, f"temp_input_{method_name}_{run_idx+1}_{sample_size}.csv")
                        temp_csv_output = os.path.join(WORK_DIR, f"temp_output_{method_name}_{run_idx+1}_{sample_size}.csv")
                        df_x.to_csv(temp_csv_input, index=False)
                        alpha = alpha_levels[method_name]

                        # Build the command for subprocess
                        command = ["Rscript", worker_script, temp_csv_input, temp_csv_output, str(alpha)]

                        # Optional: 5-minute cap if GNU timeout/gtimeout is available
                        ...
                        estimated_adj_df = to_signed_cpdag(estimated_adj_df)
